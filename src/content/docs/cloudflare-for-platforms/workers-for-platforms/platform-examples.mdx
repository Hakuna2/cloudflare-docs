...
sidebar:
  order: 8
description: Reference architectures, starter templates, and API examples for building platforms.
---
...
Build your own platform using Workers for Platforms. Start with a reference architecture, deploy a starter template, or use the API examples to build from scratch.
...
A starter template for building a platform that lets users deploy Workers. Includes subdomain routing, custom domains, and a management API.
...
Build a platform where users describe what they want and AI generates and deploys working applications. Cloudflare's [VibeSDK](https://github.com/cloudflare/vibesdk) provides the framework for building these "vibe coding" experiences, handling AI code generation, deployment to Workers for Platforms, and project management.
...
- A **dispatch namespace** - Created via the [dashboard](/cloudflare-for-platforms/workers-for-platforms/get-started/configuration/#1-create-a-dispatch-namespace)
	...
	const result =
		await client.workersForPlatforms.dispatch.namespaces.scripts.update(
			...
			scriptName,
			{
				...
				"<any part of the script>": scriptContent,
				metadata: {
...
async function listWorkers(accountId: string, namespace: string) {
	const scripts =
		await client.workersForPlatforms.dispatch.namespaces.scripts.list(
			namespace,
			{ account_id: accountId },
		);
...
To delete a single Worker by name, use:
