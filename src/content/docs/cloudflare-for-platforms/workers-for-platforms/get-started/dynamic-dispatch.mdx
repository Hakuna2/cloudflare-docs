...
Use [custom limits](/cloudflare-for-platforms/workers-for-platforms/configuration/custom-limits/) to control CPU time and subrequests for user Workers. You can set different limits based on customer plan type or other criteria.
		...
		try {
			...
			const limits =
				customerPlan === "enterprise"
					...
					: customerPlan === "pro"
						? { cpuMs: 20, subRequests: 20 }
						: { cpuMs: 10, subRequests: 5 }; // free plan
